...
def collate_test_fn(data, data_neg):
    return collate_test(data, data_neg)
...
def main():
    ...
    # Use functools.partial to bind the data_neg argument
    partial_collate_fn = functools.partial(collate_test_fn, data_neg=data_neg)
    test_data = DataLoader(dataset=test_set, batch_size=opt.batchSize, collate_fn=partial_collate_fn, pin_memory=True, num_workers=8)


    if opt.val:
        val_data = DataLoader(dataset=val_set, batch_size=opt.batchSize, collate_fn=lambda x: collate_test_fn(x, data_neg), pin_memory=True, num_workers=2)
